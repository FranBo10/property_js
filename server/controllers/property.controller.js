...
const getPropertyDetail = async (req, res) => {
    ...
    if(propertyExists) {
        ...
        res.status(404).json({ message: "La propriété n'a pas été trouvé" });
    }
};

const createProperty = async (req, res) => {
    try {
        const { title, description, propertyType, location, price, surface, bedrooms, bathrooms, wc, photo } = req.body;
    ...
    res.status(200).json({ message: 'Votre propriété est en ligne, merci !!' });  
...
const updateProperty = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'La propriété a été mise à jour' });
...
const deleteProperty = async (req, res) => {
    try {
        ...
        if(!propertyToDelete) throw new error("La propriété n'a pas était trouvé");
        const session = await mongoose.StartSession();
        session.startTransaction();
        ...
        res.status(200).json({ message: 'La propriété a été supprimé' });
